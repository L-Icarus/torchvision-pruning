...
class RankHook(FeatureHook):
    def __init__(self):
        super().__init__()
        self.feature_result = torch.tensor(0.)
        self.total = torch.tensor(0.)

    # https://github.com/lmbxmu/HRank/blob/33050a16c11b5e0f105b268be8c2a42087d11c9d/rank_generation.py#L173
    def __call__(self, module, module_in, module_out):
        a = module_out.shape[0]
        b = module_out.shape[1]
        c = torch.tensor([torch.matrix_rank(module_out[i, j, :, :]).item() for i in range(a) for j in range(b)])

        c = c.view(a, -1).float()
        c = c.sum(0)
        # 根据所有输入样本求每个filter的平均秩
...
class HRankPruner(Pruning):
    """
    HRank pruning is filter pruning algorithm
    HRank需要先为每个卷积添加hook,然后在训练集上跑一些iter来提取特征进行剪枝
    """
    ...
    def get_prune_mask(self, prune_node, prune_ratio=None):
        ...
        # 保留秩高的filter
        _, index = torch.topk(self._hook_dict[prune_node.name].get_feature(), k=save_channels)
