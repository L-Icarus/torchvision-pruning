...
class SFPMaskModel(MaskModel):
    ...
    # https://github.com/he-y/soft-filter-pruning/blob/master/pruning_cifar10_resnet.py get_filter_codebook
    def update_mask(self, opname, prune_channel_number=-1):
        ...
        if prune_channel_number != -1:
            filter_pruned_num = prune_channel_number
        else:
            filter_pruned_num = int(mask.size()[0] * self.op_prune_ratio[opname])
...
class SFPPruner(Pruning):
    """
    sfp pruning is filter pruning algorithm
    每个epoch先调用update_all_mask,再调用update_all_op_param
    """
    ...
    def __init__(self, model, op_prune_ratio=0.5):
        ...
        self.wrapped_model = SFPMaskModel(self.model, self.prune_graph, PRUNE_CONV_OPTYPE, self.op_prune_ratio,
    ...
    def get_prune_mask(self, prune_node, prune_ratio=None):
        ...
        if save_channels == math.ceil(channels * (1 - prune_ratio)):
            ...
            # param可以不需要更新,因为是结构化剪枝,之后filter会被剪掉
