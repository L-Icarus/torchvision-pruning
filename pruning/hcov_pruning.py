# https://ieeexplore.ieee.org/document/9533757
import math
import torch
...
class CovHook(FeatureHook):
    ...
    def __call__(self, module, module_in, module_out):
        batch = module_out.shape[0]
        channels = module_out.shape[1]
        x_cov = 0
        for i in range(batch):
            x_cov += torch.cov(module_out[i].view(channels, -1)).abs().sum(1)
        # 根据所有输入样本求每个filter的平均秩
        self.feature_result = self.feature_result * self.total + x_cov
        self.total += batch
...
class HCovPruner(Pruning):
    """
    HCov pruning is filter pruning algorithm
    HCov需要先为每个卷积添加hook,然后在训练集上跑一些iter来提取特征进行剪枝
    """
